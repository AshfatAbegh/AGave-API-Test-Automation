package com.agave.tests.Profile.Buyer;
...
public class BuyerDetailsTest {
    ...
    @Test(priority = 1)
    public void userShouldGetIndividualBuyerProfileWithValidId() {

        Response response = given()
                .header("Authorization", "Bearer " + accessToken)
                .header("Accept-Language", "jp")
                .when()
                .get(baseURI + "v1/profile/buyer/2");

        Assert.assertEquals(response.getStatusCode(), 200);
        response.then().assertThat()
                .body(JsonSchemaValidator.matchesJsonSchemaInClasspath("BuyerDetailsTestJSONSchema.json"));
    }

    @Test(priority = 2)
    public void userShouldNotGetIndividualBuyerProfileWithInvalidId() {

        Response response = given()
                .header("Authorization", "Bearer " + accessToken)
                .header("Accept-Language", "jp")
                .when()
                .get(baseURI + "v1/profile/buyer/4");
    ...
    @Test(priority = 3)
    public void userShouldNotGetIndividualBuyerProfileWithoutAuthorization() {

        Response response = given()
                .header("Accept-Language", "jp")
                .when()
                .post(baseURI + "v1/profile/buyer/2");
