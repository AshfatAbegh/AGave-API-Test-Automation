...
public class SellerDetailsTest {
    ...
    @Test(priority = 1)
    public void userShouldSeeSellerDetailWithValidIdAndProperAuthorization() {

        Response response = given()
                .header("Authorization", "Bearer " + accessToken)
                .when()
                .get(baseURI + "v1/profile/seller/1");

        Assert.assertEquals(response.getStatusCode(), 200);
        response.then().assertThat()
                .body(JsonSchemaValidator.matchesJsonSchemaInClasspath("SellerDetailsTestJSONSchema.json"));
    }

    @Test(priority = 2)
    public void userShouldNotSeeSellerDetailWithInvalidId() {

        Response response = given()
                .header("Authorization", "Bearer " + accessToken)
                .when()
                .get(baseURI + "v1/profile/seller/5");
    ...
    @Test(priority = 3)
    public void userShouldNotSeeSellerDetailWithoutAuthorization() {
