...
public class ProductUpdateImagesTest {
    ...
    public void dbSetUp() throws SQLException {
        ...
        insertProduct(1, "Japan", new Date().getTime(), "Grafting", "BareRoot", "Taiwan", new Date().getTime(), "Six", new Date().getTime(), "Agave", new Date().getTime(), 1, 33410, new Date().getTime(), new Date().getTime(), new Date().getTime(), "454 m.n", "Tokyo", "CashOnDelivery", "FourthClassMail", "Hokkaido", "1200", "Town", "Small", "PUBLISHED", "All Ok", "UmbrellaPlant", new Date().getTime(), "LessThanFive", 2, 1, "PD0000001");
    }
    ...
    @Test(priority = 1)
    public void userShouldUpdateProductImageWithValidProductId() {
        ...
        Response response = given()
                ...
                .multiPart("productId", 1)
                .multiPart("productImages", file)
                .multiPart("productImages", file2)
                .when()
                .put(baseURI + "v1/product/update-image");

        Assert.assertEquals(response.getStatusCode(), 200);
    }

    @Test(priority = 2)
    public void userShouldDeleteProductImageWithValidProductId() {
    ...
    @Test(priority = 3)
    public void userShouldNotUpdateAndDeleteProductImageWithInvalidProductId() {

        Response response = given()
                .header("Authorization", "Bearer " + accessToken)
                .header("Accept-Language", "jp")
                .contentType("multipart/formdata")
                .multiPart("productId", 2)
                .multiPart("deleteImageIds", 2)
                .when()
                .put(baseURI + "v1/product/update-image");
    ...
    @Test(priority = 4)
    public void userShouldNotUpdateAndDeleteProductImageWithoutAuthorization() {
