...
public class ProductListTest {
        ...
        @Test(priority = 1)
        public void sellerShouldGetProductListWithProperAuthorization() {

                Response response = given()
                                .header("Authorization", "Bearer " + accessToken)
                                .queryParam("sellerId", 1)
                                .when()
                                .get(baseURI + "v1/product/seller-products");

                Assert.assertEquals(response.getStatusCode(), 200);
                response.then().assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath("ProductListTestJSONSchema.json"));
        }
        ...
        @Test(priority = 3)
        public void sellerShouldNotGetProductListWithInvalidId() {

                Response response = given()
                                .header("Authorization", "Bearer " + accessToken)
                                .queryParam("sellerId", 4)
                                .when()
                                .get(baseURI + "v1/product/seller-products");

                Assert.assertEquals(response.getStatusCode(), 406);
                String message = response.jsonPath().getString("message");
                Assert.assertEquals(message, "販売者が存在しません");
        }
}
