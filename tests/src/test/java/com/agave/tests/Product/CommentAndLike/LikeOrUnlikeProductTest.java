...
public class LikeOrUnlikeProductTest {
    ...
    public void dbSetUp() throws SQLException {
        ...
        DB.truncateTables(new String[] { "products", "products_images", "productHashtag", "products_browsedUsers", "products_likedUsers" });

        accessToken = jwtToken.generateAccessToken(1, "asif1@gmail.com");

        insertProduct(1, "Japan", new Date().getTime(), "Grafting", "BareRoot", "Taiwan", new Date().getTime(), "Six",
                new Date().getTime(), "Agave", new Date().getTime(), 1, 33410, new Date().getTime(),
                new Date().getTime(), new Date().getTime(), "454 m.n", "Tokyo", "CashOnDelivery",
                "FourthClassMail", "Hokkaido", "1200", "Town", "Small", "PUBLISHED", "All Ok", "UmbrellaPlant",
    ...
    @Test(priority = 1)
    public void userShouldLikeProductWithValidId() {
        ...
        Response response = given()
                ...
                .body(hm)
                .when()
                .post(baseURI + "v1/product/like");

        Assert.assertEquals(response.getStatusCode(), 200);
        String message = response.jsonPath().getString("message");
        Assert.assertEquals(message, "正常に「いいね」されました");
    }

    @Test(priority = 2)
    public void userShouldNotLikeProductWithInvalidId() {
    ...
    @Test(priority = 3)
    public void userShouldNotLikeProductWithoutAuthorization() {
    ...
    @Test(priority = 4)
    public void userShouldNotLikeProductWhichIsAlreadyLiked() {
        ...
        Assert.assertEquals(response.getStatusCode(), 200);
        String message = response.jsonPath().getString("message");
        Assert.assertEquals(message, "嫌いになりました");
    }
}
